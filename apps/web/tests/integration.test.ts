...
describeIfDb('auth + admin approval flow', () => {
  ...
  it('admin salva cookies do yt-dlp: cifra, GET não vaza, valida lixo, DELETE limpa', async () => {
    await signUp('admin@voxen.local', 'senha-super-segura-123', 'Admin');
    const signin = await signIn('admin@voxen.local', 'senha-super-segura-123');
    const cookie = extractCookie(signin);
    ...
    const cookiesTxt =
      ...
      '.instagram.com\tTRUE\t/\tTRUE\t0\tsessionid\tsuper-secret-session-value\n';

    // Estado inicial: não configurado.
    const before = await app.fetch(
      new Request('http://localhost/api/admin/yt-dlp-cookies', { headers: { cookie } }),
    );
    expect(before.status).toBe(200);
    expect(((await before.json()) as { configured: boolean }).configured).toBe(false);

    // Lixo é rejeitado com 400 e NÃO persiste.
    const bad = await app.fetch(
      new Request('http://localhost/api/admin/yt-dlp-cookies', {
        method: 'PUT',
        headers: { cookie, 'content-type': 'application/json' },
        body: JSON.stringify({ cookies: 'just random garbage no tabs no header' }),
      }),
    );
    expect(bad.status).toBe(400);
    const stillEmpty = await db.setting.findFirst({
      where: { scope: 'GLOBAL', userId: null, key: 'yt_dlp_cookies' },
    });
    expect(stillEmpty).toBeNull();

    // Salva cookies válidos.
    ...
    // Persistido CIFRADO (valueEnc != conteúdo dos cookies).
    const row = await db.setting.findFirst({
      where: { scope: 'GLOBAL', userId: null, key: 'yt_dlp_cookies' },
      select: { valueEnc: true },
    });
    expect(row).not.toBeNull();
    expect(row!.valueEnc).not.toContain('super-secret-session-value');

    // GET reporta configured=true mas NUNCA devolve o valor.
    const after = await app.fetch(
      new Request('http://localhost/api/admin/yt-dlp-cookies', { headers: { cookie } }),
    );
    const afterBody = (await after.json()) as Record<string, unknown>;
    expect(afterBody.configured).toBe(true);
    expect(JSON.stringify(afterBody)).not.toContain('super-secret-session-value');
    expect(JSON.stringify(afterBody)).not.toContain('sessionid');
  ...
  it('user comum recebe 403 em /api/admin/yt-dlp-cookies', async () => {
    await signUp('admin@voxen.local', 'senha-super-segura-123', 'Admin');
    await signUp('user@voxen.local', 'senha-super-segura-456', 'User');
