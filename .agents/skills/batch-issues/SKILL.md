...
5. Limpar worktrees (ver seção abaixo) — a branch sobrevive à remoção da worktree
...
- CI vermelho após review limpo é o caso legítimo (flake, divergência de ambiente, gate que só roda no runner). A worktree do lote já foi removida no passo 5, então a correção acontece numa worktree nova cortada da branch da PR: implementar → re-revisar o commit de fix → re-empurrar → remover a worktree de novo
