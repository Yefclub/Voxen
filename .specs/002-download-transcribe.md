...
- **While** extracting audio, the system shall select a format that contains an audio track (`bestaudio/best*[acodec!=none]/best`) to avoid downloading a video-only rendition (ex.: alguns reels do Instagram) que faria o `FFmpegExtractAudio` falhar no ffprobe.
- **If** the downloaded media has no audio track (`unable to obtain file audio codec`), the system shall fail with a friendly `errorMsg` (sem traceback `job-failed-unexpected`) orientando o user a tentar de novo ou enviar por upload manual — sem retentar (falha determinística).
