...
export function missingCustomGinIndexes(indexMethods, ignoredIndexes) {
  if (
    !Array.isArray(ignoredIndexes) ||
    ignoredIndexes.some(
      (name) => typeof name !== "string" || !/^[A-Za-z0-9_]+$/.test(name),
  ...
  if (!(indexMethods instanceof Map)) {
    throw new Error("Index catalog evidence must be a Map.");
  }
  return ignoredIndexes.filter(
    (name) => indexMethods.get(name)?.toLowerCase() !== "gin",
  );
}
