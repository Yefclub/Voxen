...
function loadStrict(path = FILE) {
  ...
  try {
    ...
    throw new Error(
      `[changelog] não consegui ler ${path}; promoção cancelada.`,
    );
  }
  let data;
  try {
    ...
    throw new Error(
      `[changelog] ${path} contém JSON inválido; promoção cancelada.`,
    );
  }
  if (!Array.isArray(data)) {
    throw new Error(
      `[changelog] ${path} precisa conter uma lista; promoção cancelada.`,
    );
  }
  return data;
}
...
function parseLocalizedBody(body) {
  const [english, ...portugueseParts] = body.split(
    /\r?\n<!--[ \t]*pt-BR[ \t]*-->\r?\n/i,
  );
  const portuguese = portugueseParts.join("\n").trim();
  return {
...
/** @returns {{ type: string, title: string, body: string, translations?: Record<string, { title?: string, body?: string }> } | null} */
export function parseChangelogFile(text) {
  ...
  if (!m) return null;
  const fm = m[1];
  const localizedBody = parseLocalizedBody(m[2]);
  let type = frontmatterValue(fm, "tipo").toLowerCase();
  const titleEn = frontmatterValue(fm, "titulo_en");
  const titlePtBr = frontmatterValue(fm, "titulo_pt_br");
  const legacyTitle = frontmatterValue(fm, "t[ií]tulo");
  const title = titleEn || legacyTitle;
  const body = localizedBody.english;
  if (!TYPES.includes(type)) type = "chore";
  if (!title || !body) return null;
  if (!titleEn && !titlePtBr && !localizedBody.portuguese)
    return { type, title, body };
  const translations = {
    ...(titleEn || body
      ? { en: { ...(titleEn ? { title: titleEn } : {}), body } }
      : {}),
    ...(titlePtBr || localizedBody.portuguese
      ? {
          "pt-BR": {
            ...(titlePtBr ? { title: titlePtBr } : {}),
            ...(localizedBody.portuguese
              ? { body: localizedBody.portuguese }
              : {}),
          },
        }
      : {}),
  };
  return { type, title, body, translations };
}
...
function addDev() {
  ...
  if (!note) {
    console.error(
      `[changelog] ${source} inválido (frontmatter/corpo) — pulando.`,
    );
    return false;
  }
  const entries = load();
  const entry = {
    ...
    ...(note.translations ? { translations: note.translations } : {}),
  };
...
function addProd() {
  ...
  if (curated) {
    ...
    if (curated.translations) entry.translations = curated.translations;
  }
...
function mergeKeepDev() {
  ...
  if (!oursPath || !fs.existsSync(oursPath)) {
    console.log(
      "[changelog] merge-keep-dev: RN_OURS ausente — nada a preservar.",
    );
    return true;
  }
  const main = load();
  ...
  console.log(
    `[changelog] merge-keep-dev: preservou ${devOnly.length} entrada(s) dev pós-corte`,
  );
  return true;
}
